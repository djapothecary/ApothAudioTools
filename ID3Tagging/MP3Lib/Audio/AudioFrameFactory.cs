...
namespace ID3Tagging.MP3Lib.Audio
{
    ...
    internal static class AudioFrameFactory
    {
        ...
        /// </summary>
        /// <param name="stream">
        /// source stream, advanced by length of the frame on read
        /// </param>
        /// <param name="remainingBytes">
        /// number of bytes in audio block, as reported by the caller
        /// </param>
        /// <returns>
        /// wrapper for derived type of AudioFrame
        /// </returns>
        public static AudioFrame CreateFrame(Stream stream, uint remainingBytes)
        {
            ...
            if (header.IsFreeBitRate)
            {
                ...
                frameFullSize = firstFrameHdrSize
            ...
            else
            {
                ...
                Trace.Assert(frameLengthInBytes != null, "header.FrameLengthInBytes != null");
                if (header.FrameLengthInBytes != null)
                {
        ...
        /// </param>
        /// <returns>
        /// number of bytes skipped, not length of frame found!
        /// </returns>
        ...
        static AudioFrameHeader CreateHeader(Stream stream, uint remainingBytes)
        {
            ...
            long endPos = streamStartPosition + remainingBytes;
            ...
            do
            {
                ...
                {
                    if (frameStartPos > streamStartPosition)
                    {
                        if (framesSkipped > 0)
                        {
                            Trace.WriteLine(
                                string.Format(
                                    "total {0} bytes and {1} invalid frame headers skipped to get to the start of a valid frame at stream offset {2}",
                                    frameStartPos - streamStartPosition,
                                    framesSkipped,
                                    frameStartPos));
                        }
                        else
                        {
                            Trace.WriteLine(
                                string.Format(
                                    "total {0} bytes skipped to get to the start of a valid frame at stream offset {1}",
                                    frameStartPos - streamStartPosition,
                                    frameStartPos));
                        }
                    }
        ...
        /// </summary>
        /// <param name="stream">
        /// The stream to perform the search on.
        /// </param>
        /// <param name="remainingBytes">
        /// </param>
        /// <returns>
        /// number of bytes skipped; stream is at first mp3 header position.
        /// </returns>
        ...
        /// a byte array
        /// </returns>
        /// <exception cref="InvalidAudioFrameException"></exception>
