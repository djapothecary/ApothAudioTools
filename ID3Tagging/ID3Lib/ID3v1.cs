...
namespace ID3Tagging.ID3Lib
{
    ...
    public class ID3v1
    {
        ...
        private static readonly string[] _genres =
        {
            ...
            "Christian Rock", "Merengue", "Salsa", "Trash Metal", "Anime", "JPop", "SynthPop", "UKG"
        };
        ...
        private TagModel GetFrameModel()
        {
            ...
            frameFullText.Text = this.Comment;
            frameModel.Add(frameFullText);

            if (this.Genre < _genres.Length)
            {
                ...
                frameText.TextCode = TextCode.Ascii;
                frameText.Text = _genres[this.Genre];
                frameModel.Add(frameText);
            }
        ...
        private void SetFrameModel(IEnumerable<FrameBase> frameModel)
        {
            Clear();
            foreach (var frame in frameModel)
            {
                ...
                {
                    case "TIT2":
                        this.Song = GetTagText(frame);
                        break;
                    case "TPE1":
                        this.Artist = GetTagText(frame);
                        break;
                    case "TALB":
                        this.Album = GetTagText(frame);
                        break;
                    case "TYER":
                        this.Year = GetTagText(frame);
                        break;
                    case "TRCK":
                        ...
                        break;
                    case "TCON":
                        this.Genre = ParseGenre(GetTagText(frame));
                        break;
                    case "COMM":
                        this.Comment = GetTagText(frame);
                        break;
                }
            }
        }
        ...
        public void Deserialize(Stream stream)
        {
            ...
            this.Song = GetString(encoding, tag);

            reader.Read(tag, 0, 30);
            this.Artist = GetString(encoding, tag);

            reader.Read(tag, 0, 30);
            this.Album = GetString(encoding, tag);

            reader.Read(tag, 0, 4);
            this.Year = (tag[0] != 0 && tag[1] != 0 && tag[2] != 0 && tag[3] != 0) ? encoding.GetString(tag, 0, 4) : string.Empty;

            reader.Read(tag, 0, 30);
            if (tag[28] == 0)
            {
                // Track number was stored at position 29 later hack of the original standard.
                _track = tag[29];
                this.Comment = encoding.GetString(tag, 0, Memory.FindByte(tag, 0x00, 0));
            }
            else
            {
                _track = 0;
                this.Comment = GetString(encoding, tag);
            }
        ...
        public void Write(Stream stream)
        {
            ...
            {
                this.Song = this.Song.Substring(0, 30);
            }

            encoding.GetBytes(this.Song, 0, this.Song.Length, tag, 0);
            ...
            encoding.GetBytes(this.Artist, 0, this.Artist.Length, tag, 0);
            writer.Write(tag, 0, 30);
            Memory.Clear(tag, 0, 30);

            if (this.Album.Length > 30)
            {
                this.Album = this.Album.Substring(0, 30);
            }

            encoding.GetBytes(this.Album, 0, this.Album.Length, tag, 0);
            writer.Write(tag, 0, 30);
            Memory.Clear(tag, 0, 30);
            ...
            {
                ushort year;
                if (!ushort.TryParse(this.Year, out year))
                {
                    this.Year = "0";
                }                    
                ...
                encoding.GetBytes(this.Year, 0, this.Year.Length, tag, 0);
            }
            ...
            {
                this.Comment = this.Comment.Substring(0, 28);
            }                

            encoding.GetBytes(this.Comment, 0, this.Comment.Length, tag, 0);

            writer.Write(tag, 0, 28);
            Memory.Clear(tag, 0, 30);
            writer.Write((byte)0);
            writer.Write(this._track);
