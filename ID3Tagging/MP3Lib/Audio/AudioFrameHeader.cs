...
namespace ID3Tagging.MP3Lib.Audio
{
    ...
    public class AudioFrameHeader
    {
        ...
        private static readonly uint[] V1L1 =
        {
            0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, uint.MaxValue
        };

        private static readonly uint[] V1L2 =
        {
            0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, uint.MaxValue
        };

        private static readonly uint[] V1L3 =
        {
            0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, uint.MaxValue
        };

        private static readonly uint[] V2L1 =
        {
            0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256, uint.MaxValue
        };

        private static readonly uint[] V2L2L3 =
        {
            0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, uint.MaxValue
        };

        // Frequency sampling speed, indexed by RawSampleFreq
        private static readonly uint[] MPEG1 = { 44100, 48000, 32000, uint.MaxValue };

        private static readonly uint[] MPEG2 = { 22050, 24000, 16000, uint.MaxValue };

        private static readonly uint[] MPEG25 = { 11025, 12000, 8000, uint.MaxValue };

        // AudioFrame size in samples, indexed by RawLayer
        // 0 (illegal)    1=L3  2=L2  3=L1
        private static readonly uint[] V1Size = { uint.MaxValue, 1152, 1152, 384 };

        private static readonly uint[] V2Size = { uint.MaxValue, 576, 1152, 384 };

        // Channel Mode names, indexed by ChannelMode
        private static readonly string[] MpegChannelMode = {
                                                               "Stereo", "Joint Stereo", "Dual channel (2 mono channels)",
                                                               "Single channel (Mono)"
                                                           };

        // Layer names, indexed by RawLayer
        private static string[] mpegLayer = { "Reserved", "Layer III", "Layer II", "Layer I" };

        // Emphasis names, indexed by Emphasis
        private static readonly string[] MpegEmphasis = { "None", "50/15 ms", "Reserved", "CCIT J.17" };
        ...
        private byte RawMpegVersion
        {
            get
            {
                return (byte)((_headerBuffer[1] & 0x18) >> 3);
            }
        } // ........ ...XX... ........ ........      
        // Layer description
        private byte RawLayer
        {
            get
            {
                return (byte)((_headerBuffer[1] & 0x06) >> 1);
            }
        } // ........ .....XX. ........ ........      
        ...
        private byte RawBitRate
        {
            get
            {
                return (byte)((_headerBuffer[2] & 0xf0) >> 4);
            }
        } // ........ ........ XXXX.... ........
        ...
        private byte RawSampleFreq
        {
            get
            {
                return (byte)((_headerBuffer[2] & 0x0C) >> 2);
            }
        } // ........ ........ ....XX.. ........
        // Padding bit
        ...
        private byte RawChannelMode
        {
            get
            {
                return (byte)((_headerBuffer[3] & 0xC0) >> 6);
            }
        } // ........ ........ ........ XX......   
        ...
        private byte RawModeExtension
        {
            get
            {
                return (byte)((_headerBuffer[3] & 0x30) >> 4);
            }
        } // ........ ........ ........ ..XX....   
        // Copyright
        ...
        private byte RawEmphasis
        {
            get
            {
                return (byte)(_headerBuffer[3] & 0x03);
            }
        } // ........ ........ ........ ......XX   
        ...
        public bool Valid
        {
            get
            {
                return RawMpegVersion != 1 && RawLayer != 0 && RawBitRate != 15 && RawSampleFreq != 3 && RawBitRate != 15;
            }
        }
                    ...
                    case 0:
                        {
                            ...
                            {
                                case 1: // Layer III
                                    retval = V2L2L3[RawBitRate];
                                    break;
                                case 2: // Layer II
                                    retval = V2L2L3[RawBitRate];
                                    break;
                                case 3: // Layer I
                                    retval = V2L1[RawBitRate];
                                    break;
                                default:
                    ...
                    case 3:
                        {
                            ...
                            {
                                case 1: // Layer III
                                    retval = V1L3[RawBitRate];
                                    break;
                                case 2: // Layer II
                                    retval = V1L2[RawBitRate];
                                    break;
                                case 3: // Layer I
                                    retval = V1L1[RawBitRate];
                                    break;
                                default:
        ...
        public uint SamplesPerSecond
        {
            get
            {
                ...
                {
                    case 0: // MPEG 2.5
                        retval = MPEG25[RawSampleFreq];
                        break;
                    case 2: // MPEG 2
                        retval = MPEG2[RawSampleFreq];
                        break;
                    case 3: // MPEG 1
                        retval = MPEG1[RawSampleFreq];
                        break;
                    default:
                        throw new InvalidAudioFrameException("MPEG Version not recognised");
                }
        ...
        public uint SamplesPerFrame
        {
            get
            {
                ...
                switch (RawMpegVersion)
                {
                    case 0: // MPEG 2.5
                    case 2: // MPEG 2
                        retval = V2Size[RawLayer];
                        break;
                    case 3: // MPEG 1
                        retval = V1Size[RawLayer];
                        break;
                    default:
                        throw new InvalidAudioFrameException("MPEG Version not recognised");
                }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AudioFrameHeader"/> class. 
