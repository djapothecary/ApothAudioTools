...
namespace Id3.Net.Internal
{
    internal sealed class AudioStream
    {
        ...
        internal AudioStreamProperties Calculate()
        {
            ...
            do
            {
                _bitHeader = (ulong)(((_audioStream[position] & 255) << 24) |
                    ((_audioStream[position + 1] & 255) << 16) |
                    ((_audioStream[position + 2] & 255) << 8) |
        ...
        private void CheckVariableBitrateHeader(byte[] header)
        {
            ...
            if ((flags & 0x0001) == 1)
            {
                _variableFrames = (int)(((header[8] & 255) << 24) |
                    ((header[9] & 255) << 16) |
                    ((header[10] & 255) << 8) |
                    ((header[11] & 255)));
        ...
        private int Bitrate
        {
            get
            {
                if (_isVariableBitrate)
                {
                    ...
                    return (int)((medFrameSize * (double)Frequency) / (1000.0 * ((LayerIndex == 3) ? 12.0 : 144.0)));
                }
        ...
        private TimeSpan Duration
        {
            get
            {
                var kilobitFileSize = (int)((8 * _audioStream.Length) / 1000);
                var seconds = (int)(kilobitFileSize / Bitrate);
        ...
        private int Frequency
        {
            get
            {
                return FrequencyLookup[VersionIndex, FrequencyIndex];
            }
        }
        ...
        private int FrameCount
        {
            get
            {
                if (!_isVariableBitrate)
                {
                    var medFrameSize = (double)(((LayerIndex == 3) ? 12 : 144) * ((1000.0 * (float)Bitrate) / (float)Frequency));
